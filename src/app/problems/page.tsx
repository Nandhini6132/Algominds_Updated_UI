...
const ProblemPage = () => {
  const { isAdmin, sortOrder, setSortOrder, category, setCategory }: any =
  ...
  //     const optionsWithCounts = Object.entries(categoryCounts).map(
  //       ([category, count]) => ({
  //         category,
    ...
    <main className="p-4  w-[50%] m-auto">
      <div className="flex gap-6 mb-7">
        {options.map(({ category, count }) => {
          return (
            <Link href={""} key={count}>
              ...
              >
                <div className="hover:text-blue-500 ">{`${category
                  .charAt(0)
                  .toUpperCase()}${category.slice(1)}`}</div>
        ...
        >
          ...
          <span className="bg-gray-200 px-2 text-sm rounded-xl flex items-center">
            {totalQuestionLength}
          </span>
        </div>
      </div>
